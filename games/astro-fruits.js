...
let initialStage = 1; // משתנה גלובלי עבור השלב ההתחלתי
...
function loadLevel(levelNum) {
    ...
    if (levelData[levelNum]) {
        ...
        // לוודא שמופע ה-p5 זמין לפני יצירת אובייקטים
        const p = window.p5Instance;
        if (p) {
            player = new Player(currentLevelData.playerStart.x, currentLevelData.playerStart.y, levelNum, p);
            platforms = currentLevelData.platforms.map(pData => new Platform(pData.x, pData.y, pData.w, pData.h, p));
            collectibleFruits = currentLevelData.fruits.map(fData => new CollectibleFruit(fData.x, fData.y, fData.type, p));
            exitPoint = new ExitPoint(currentLevelData.exit.x, currentLevelData.exit.y, currentLevelData.exit.w, currentLevelData.exit.h, p);
            activeKeys = {};
        }
    } else {
        gamePhase = 'game_over';
...
const sketch = function(p) {
    // חשוב: אנחנו שומרים את מופע ה-p5 במשתנה גלובלי כדי ששאר הקוד יוכל להשתמש בו.
    window.p5Instance = p;
    ...
    function drawUI() {
        ...
        p.textAlign(p.RIGHT);
        if (player && currentLevelData && player.currentLevel < Object.keys(levelData).length) {
            p.text(`פירות בשלב: ${currentLevelFruits}/${currentLevelData.fruits.length}`, p.width - 10, 20);
        } else if (player && currentLevelData && player.currentLevel === Object.keys(levelData).length) {
             p.text(`פירות בשלב: ${currentLevelFruits}/${currentLevelData.fruits.length}`, p.width - 10, 20);
        }
...
// *** זהו השינוי העיקרי! ***
// אנחנו מפעילים את הקוד הזה רק אחרי שכל הדף נטען.
// זה מבטיח שהפונקציה 'sketch' תוגדר לפני ש-p5 מנסה להשתמש בה.
window.onload = function() {
    const gameContainerElement = document.getElementById('gameContainer');
    if (gameContainerElement) {
        window.p5Instance = new p5(sketch, gameContainerElement);
    } else {
        console.error("אלמנט ה-gameContainer לא נמצא. לא ניתן לאתחל את p5.js.");
    }
};
