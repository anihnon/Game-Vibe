...
let initialStage = 1; // **תיקון: הגדרת המשתנה החסר**
...
// --- פונקציה להצגת תיבת הודעה מותאמת אישית (במקום alert) ---
...
class Player {
    ...
    update() {
        ...
        const isLeft = activeKeys[this.p.LEFT_ARROW] || activeKeys[65]; // חץ שמאלה או A
        const isRight = activeKeys[this.p.RIGHT_ARROW] || activeKeys[68]; // חץ ימינה או D
        ...
        if (this.currentLevel >= GRAVITY_TOGGLE_LEVEL && activeKeys[32] && !spacePressed) { // מקש רווח = 32
...
// --- נתוני השלבים (מופשטים לצורך הדגמה) ---
const levelData = {
    1: {
    ...
    3: { // שלב שבו ניתן לשלוט בכוח המשיכה
        playerStart: { x: 100, y: 100 },
        platforms: [
            { x: 300, y: 500, w: 600, h: 50 },
            { x: 150, y: 400, w: 100, h: 20 },
...
const sketch = function(p) {
    ...
    // פונקציית עזר ליצירת פיקסל ארט (חדש)
    ...
    p.preload = function() {
        ...
        GameAssets.fontGlyphs = [
            // A - קוד צבע ונקודות
            createPixelArt(1, [{ color: [255, 255, 255], points: [[0, -2], [1, -1], [-1, -1], [0, 0], [-1, 1], [1, 1]] }]),
            // B
            createPixelArt(1, [{ color: [255, 255, 255], points: [[0, -2], [1, -2], [0, -1], [1, -1], [0, 0], [1, 0], [0, 1], [1, 1]] }]),
            // ... יש להשלים את שאר האותיות
        ];
    };
    ...
    p.draw = function() {
        ...
        if (gamePhase === 'playing') {
            ...
            for (let i = collectibleFruits.length - 1; i >= 0; i--) {
                const fruit = collectibleFruits[i];
            ...
            if (player.collidesWith(exitPoint) && collectibleFruits.length === 0) {
                gamePhase = 'level_complete';
                showMessageBox('כל הכבוד!', `השלמת את שלב ${player.currentLevel} בהצלחה!`);
            }
            // הצגת מידע על המשחק
            p.fill(255);
            p.textSize(12);
            p.textAlign(p.LEFT);
            p.text(`פירות: ${totalFruitsCollected}`, 10, 20);
            p.textAlign(p.CENTER);
            p.text(`שלב: ${player.currentLevel}`, p.width / 2, 20);
            p.text(`כוח משיכה: ${player.gravityEnabled ? 'פעיל' : 'כבוי'}`, p.width / 2, 40);
    ...
    function drawIntroScreen() {
        ...
        p.textSize(16);
        p.text('לחץ על מקש כלשהו כדי להתחיל', p.width / 2, p.height / 2);
        p.text(`פירות שנאספו: ${totalFruitsCollected}`, p.width / 2, p.height / 2 + 30);
    }
    ...
    function drawLevelCompleteScreen() {
        ...
        p.fill(255);
        p.textSize(32);
        p.text('שלב הושלם!', p.width / 2, p.height / 2 - 100);
        p.textSize(16);
        p.text(`אספת ${currentLevelFruits} פירות בשלב זה.`, p.width / 2, p.height / 2);
        p.text('לחץ על מקש כלשהו כדי לעבור לשלב הבא', p.width / 2, p.height / 2 + 50);
    }
    ...
    function drawGameOverScreen() {
        ...
        p.fill(255);
        p.textSize(32);
        p.text('המשחק נגמר!', p.width / 2, p.height / 2 - 100);
        p.textSize(16);
        p.text('לחץ על מקש כלשהו כדי להתחיל מהתחלה', p.width / 2, p.height / 2);
    }
    ...
    function drawInstructionsScreen() {
        ...
        p.fill(255);
        p.textSize(32);
        p.text('הוראות', p.width / 2, p.height / 2 - 200);
        p.textSize(16);
        let y = p.height / 2 - 150;
        const instructions = [
            'השתמשו בחצים או במקשי WASD כדי לנוע.',
            'קפצו בעזרת חץ למעלה או מקש W.',
            'תוכלו לבצע קפיצה כפולה.',
            'אספו את כל הפירות כדי לפתוח את השער.',
            'הגיעו לשער כדי לעבור לשלב הבא.'
        ];
        if (player && player.currentLevel >= GRAVITY_TOGGLE_LEVEL) {
            instructions.push('שלב 3 ואילך: לחצו על מקש רווח כדי להחליף את כוח המשיכה.');
        }

        p.textAlign(p.CENTER);
        for (const instr of instructions) {
            p.text(instr, p.width / 2, y);
            y += 30;
        }
    ...
    function drawAchievementsScreen() {
        ...
        p.fill(255);
        p.textSize(32);
        p.text('הישגים', p.width / 2, p.height / 2 - 200);
        p.textSize(16);
        let y = p.height / 2 - 150;
        const achievementsList = {
            'fruitCollector': 'אספן פירות: אספת 10 פירות!',
            'superJumper': 'קפצן על: השלמת שלב 5!',
            'skilledExplorer': 'חוקר מיומן: השלמת שלב 10!'
        };
        for (const [key, desc] of Object.entries(achievementsList)) {
            const isUnlocked = unlockedAchievements[key];
            p.fill(isUnlocked ? 0 : 255);
    ...
    // שינוי גודל הקנבס בהתאם למימדי החלון
